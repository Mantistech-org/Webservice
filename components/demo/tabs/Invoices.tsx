'use client'
// DEMO ONLY — No real API calls. All state is local.
...
const MOCK_INVOICES: Invoice[] = [
  {
    ...
    service_address: '4821 Kavanaugh Blvd, Little Rock AR',
    line_items: [
      { description: 'AC Capacitor Replacement', quantity: 1, unit_price: 45, total: 45 },
      { description: 'Refrigerant R-410A (2 lbs)', quantity: 2, unit_price: 65, total: 130 },
      { description: 'Labor - Standard (2 hours)', quantity: 2, unit_price: 95, total: 190 },
    ],
    subtotal: 365,
    tax_rate: 0,
    tax_amount: 0,
  ...
  },
  {
    ...
    customer_email: 'ray.d@outlook.com',
    service_address: '7713 Geyer Springs Rd, Little Rock AR',
    line_items: [
      { description: 'Furnace Inspection and Tune-Up', quantity: 1, unit_price: 129, total: 129 },
      { description: 'Filter Replacement', quantity: 1, unit_price: 18, total: 18 },
      { description: 'Labor - Standard (1 hour)', quantity: 1, unit_price: 95, total: 95 },
    ],
    subtotal: 242,
    tax_rate: 0,
    tax_amount: 0,
  ...
  {
    id: '3',
    customer_name: 'Michelle Carter',
    customer_email: 'm.carter@gmail.com',
    service_address: '2204 N University Ave, Little Rock AR',
    line_items: [
      { description: 'Heat Pump Annual Maintenance', quantity: 1, unit_price: 149, total: 149 },
      { description: 'Labor - Standard (1.5 hours)', quantity: 1.5, unit_price: 95, total: 142.50 },
    ],
    subtotal: 291.50,
    tax_rate: 0,
    tax_amount: 0,
...
export default function DemoInvoices({ sessionId: _sessionId, businessName: _businessName }: { sessionId: string; businessName: string }) {
  ...
  const handleCopyLink = (id: string) => {
    navigator.clipboard.writeText('#demo-payment-link').catch(() => {})
    setCopied(id)
                      ...
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                        ...
                        {inv.stripe_payment_link && (
                          <button
                            ...
                            style={{ fontSize: 12, fontWeight: 600, color: copied === inv.id ? '#00C27C' : '#7c3aed', background: 'none', border: `1px solid ${copied === inv.id ? 'rgba(0,194,124,0.3)' : 'rgba(124,58,237,0.25)'}`, borderRadius: 6, padding: '5px 12px', cursor: 'pointer', fontFamily: 'inherit' }}
                          >
                            {copied === inv.id ? 'Payment link copied.' : 'Payment Link'}
                          </button>
                        )}
                        <button
      ...
      {subTab === 'payments' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
            {[
              { label: 'Revenue This Month', value: '$365.00'              },
                        ...
                        <td style={{ padding: '14px 20px' }}>
                          {inv.stripe_payment_link ? (
                            <button
                              ...
                              style={{ fontSize: 12, fontWeight: 600, color: copied === inv.id ? '#00C27C' : '#7c3aed', background: 'none', border: `1px solid ${copied === inv.id ? 'rgba(0,194,124,0.3)' : 'rgba(124,58,237,0.25)'}`, borderRadius: 6, padding: '5px 12px', cursor: 'pointer', fontFamily: 'inherit' }}
                            >
                              {copied === inv.id ? 'Payment link copied.' : 'Open Link'}
                            </button>
                          ) : (
